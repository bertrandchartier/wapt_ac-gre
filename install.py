# -*- coding: utf-8 -*-
# -----------------------------------------------------------------------
#    This file is fork of a part of WAPT
...
#                wapt_password_ok = True
    ...
    password = pbkdf2_sha256.hash(wapt_password.encode('utf8')) #changement indentation
    server_config['wapt_password'] = password #changement indentation
...
#        pass
    ...
    reply = 'OK' # au lieu de postconf.yesno("Do you want to configure nginx?")
    if reply == 'OK': #au lieu de postconf.DIALOG_OK:
        try:
            fqdn = socket.getfqdn()
            if not fqdn:
                fqdn = 'wapt'
